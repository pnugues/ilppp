...
if __name__ == '__main__':
    ...
    w0_range = np.linspace(w0_opt, w0_opt, 100)
    w1_range = np.linspace(-0.25, -0.1, 100)
    w2_range = np.linspace(1., 4., 200)
    """
    w0_range = np.linspace(-0.0019, -0.0019, 100)
    w1_range = np.linspace(-1.5, -1., 100)
    w2_range = np.linspace(16.5, 20.5, 100)
    """
    ...
    for i in range(len(w1_range)):
        for j in range(len(w2_range)):
            z_axis[j, i] = np.array(compute_likelihood(X, y, w0_opt, w1_range[i], w2_range[j]))
